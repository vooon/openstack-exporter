...
func collectMetrics(root string) ([]metricInfo, error) {
	...
	for _, file := range files {
		...
		if exporterName == "nova" {
			labels, ok, err := stringSliceAssignment(parsed, "novaServerStatusBaseLabels")
			if err != nil {
				return nil, fmt.Errorf("%s: %w", file, err)
			}
			if !ok {
				return nil, fmt.Errorf("%s: novaServerStatusBaseLabels not found", file)
			}
		...
		if exporterName == "ironic" {
			labels, ok, err := stringSliceAssignment(parsed, "ironicNodeBaseLabels")
			if err != nil {
				return nil, fmt.Errorf("%s: %w", file, err)
			}
			if !ok {
				return nil, fmt.Errorf("%s: ironicNodeBaseLabels not found", file)
			}
			metrics = append(metrics, newMetric(exporterName, "node", labels, false, "", "labels can be extended with `--ironic.node-extra-labels`"))
		}
	}
